     ...
     console.log("some error ");
      ...
      <Subheader inset={true}>Files</Subheader>
        {filenamesDiv}
          ...
          <MuiThemeProvider>
               <RaisedButton disabled={self.state.printButtonDisabled} label="Print Files" primary={true} style={style} onClick={(event) => self.handlePrinting(event)}/>
          </MuiThemeProvider>
          </center>
          </div>
        );
...
handlePrinting(event){
  var filesToBePrinted = this.state.filesToBePrinted
  var req = request.post(apiBaseUrl+'pastFilesPrint')
                   .send({ filesArray: filesToBePrinted })
